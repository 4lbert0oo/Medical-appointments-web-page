...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $apellidos = isset($_GET['apellidos']) ? limpia($_GET['apellidos']) : null;
  $documento_id = isset($_GET['DNI']) ? limpia($_GET['DNI']) : null;
  $tipo = isset($_GET['tipo']) ? limpia($_GET['tipo']) : null;
  $direccion = isset($_GET['direccion']) ? limpia($_GET['direccion']) : null;
  $localidad = isset($_GET['localidad']) ? limpia($_GET['localidad']) : null;
  $provincia = isset($_GET['provincia'])? limpia($_GET['provincia']) : null;
  $pais = isset($_GET['pais'])? limpia($_GET['pais']) : null;
  ...
  $id=isset($_GET['id'])? limpia($_GET['id']) : null;
    ...
    }else if(validaRequerido($documento_id) and  !validar_dni($documento_id,$id,$tipo)){
    ...
    if(!$errores){
      $resultado=modificar_paciente($nombre,$apellidos,$documento_id,$tipo,$fecha_n,$direccion,$localidad,$provincia,$pais,$id);
      if($resultado){
        //header('Location:lista_pacientes_med.php');
        $errores[] = "Paciente modificado";
...
<!DOCTYPE html>
<html lang="es">
<link rel="stylesheet" type="text/css" href="css/mod_pacie.css" />
    <head>
      <meta charset="UTF-8">
      ...
      <body>
        ...
        <form action="modificar_paciente.php" id="form" method="GET">
            <table id="tabla">
              <tr>
                <td ><a>Nombre</a></td>
              ...
              <tr>
                <td><a>Apellidos</a></td>
                <td><input type="text" name="apellidos" value="<?php echo $apellidos; ?>"/><a id="cosa">*</a></td>
              </tr>
              <tr>
              ...
              <tr>
                <td><a>Direccion</a></td>
                <td>  <input type="text" name="direccion" value="<?php echo $direccion; ?>"/><a id="cosa">*</a></td>
              </tr>
              <tr>
                <td>Pais</td>
                <td>
              ...
              <tr>
                ...
                <td>
                  <select class="selectores" id="provincias" onchange="javascript:cargaMunicipios()"   name="provincia" >
                    <?php
                        echo '<option value="'.$provincia.'" >'.buscar_provincia($provincia).'</option>';
                    ?>
                  </select>
              ...
              <tr>
                ...
                <td>
                  <select class="selectores" id="municipio" name="localidad">
                      <option value=" <?php echo $localidad; ?>" ><?php echo buscar_municipio($localidad); ?></option>
                  </select>
            ...
            <input id="Registrar" type="submit" title=": D" name="modificar" onclick="return confirm('¿Quieres guardar los cambios?')"  value="Guardar cambios">
        </form>
        <?php if ($errores): ?>
            <ul style="color: #f00;">
        ...
        <?php endif; ?>
        <form action="lista_pacientes_med.php" method="post">
